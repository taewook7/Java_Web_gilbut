...
public class MemberDAO {
	...
	public List<MemberVO> selectMemberByPwd(int pwd) {
		...
		//비밀번호가 같은회원은 여러명일수있으니 selectList() 메서드로 조회한다
		membersList=session.selectList("maaper.member.selectMemberByPwd",pwd);
		return membersList;
	}
	...
	public int insertMember(MemberVO memberVO) {
		...
		SqlSession session=sqlMapper.openSession();
		int result=0;
		// 지정한 id의 SQL문에 memberVO의 값을 전달하여 회원정보를 테이블에 추가합니다.
		result=session.insert("mapper.member.insertMember",memberVO);
		session.commit();  // 수동 커밋이므로 반드시 commit() 메서드를 호출하여 영구 반영합니다.
		return result;
	}
